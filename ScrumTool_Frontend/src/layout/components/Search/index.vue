    ...
    <div id="search" :class="{'searching': isShow}" @click="isShow && $eventBus.emit('global-search-toggle')">
        <div class="container">
            <div class="search-box" @click.stop>
                <el-input ref="input" v-model="searchInput" prefix-icon="el-icon-search" placeholder="You can search pages by inputing URL, title and other related text" clearable @keydown.esc="$eventBus.emit('global-search-toggle')" @keydown.up.prevent="keyUp" @keydown.down.prevent="keyDown" @keydown.enter.prevent="keyEnter" />
                <div v-if="$store.state.settings.mode === 'pc'" class="tips">
                    <div class="tip">
                        <span>Alt</span>+<span>S</span>
                        Call the searching panel
                    </div>
                    <div class="tip">
